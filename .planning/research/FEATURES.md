...
**Domain:** Academic report-first analysis of Trending YouTube Video Statistics with Markdown checkpoints
**Researched:** 2026-04-19
**Scope:** All country CSV files in `data/`; final deliverable is a teacher-ready Vietnamese PDF, not a frontend app, with Markdown checkpoints kept as the checkpoint artifact
**Confidence:** HIGH for assignment-fit recommendations, MEDIUM for optional differentiators

A strong project in this domain should optimize for defensible analysis, not breadth for its own sake. The best final report is the one that answers the required questions clearly, uses the dataset honestly, and converts findings into practical recommendations for a new YouTube channel, while Markdown checkpoints stay supporting artifacts.
...
|---------|--------------|------------|--------------------|-------|
| Multi-country data ingestion and harmonization | The assignment scope is all country files, so the project must combine them into one analysis-ready dataset with a country identifier. | Medium | Iterating through all CSVs, schema consistency checks, type coercion, missing-value handling | Parse `trending_date` and `publish_time`, standardize booleans, and preserve country as a first-class feature. |
...
| Clear academic report structure with Markdown checkpoints | Even good analysis can score poorly if the narrative is weak. | Low | All prior analyses, plotting outputs, written interpretation | Minimum structure: question, data, preprocessing, method, findings, recommendations, limitations, conclusion. |
...
| Reproducible output packaging | Makes the submission cleaner and easier to defend if asked to rerun or explain steps. | Low-Medium | Stable script/checkpoint flow, cleaned-data export, figure generation | Examples: a saved cleaned dataset, reproducible commands, and figures with consistent titles/captions. |
...
|--------------|-----------|-------------------|
| Frontend dashboard or web app | The brief is report-first, and UI work will consume time that should go into analysis quality. | Build clear Markdown checkpoints and publication-quality figures for the final report. |
...
That combination is strong enough for an academic submission because it is aligned with the assignment, feasible within a script-and-Markdown workflow, and rigorous about what the dataset can and cannot support.

...
**Project:** Trending YouTube Video Statistics report-first workflow with Markdown checkpoints
...
- **Phase 1 - Research Design and Scope Lock:** define the exact questions the checkpoint/report flow can answer with this dataset.
...
**Consequences:** Statistical summaries become biased, feature importance is inflated, and the analysis may mistake persistence on the trending list for broader popularity.
...
- The analysis switches between UTC hours, local hours, and market-country assumptions without saying so.
...
**What goes wrong:** The dataset contains videos that already trended. Without a non-trending comparison group, the checkpoint/report flow cannot estimate the true probability of trending or make strong causal claims about why a video entered the trending list.
...
- The analysis does not distinguish pre-publish features from post-exposure outcomes.
...
**Why it happens:** The assignment asks for sentiment analysis, and basic examples usually assume English text.
...
**Consequences:** The analysis overclaims psychological insight and may say things like "audiences prefer positive sentiment" when it actually measured title tone or tag wording.
...
**Consequences:** The analysis may conclude that one market "performs better" when it is mostly measuring market size, channel scale, or platform penetration.
...
**Why it happens:** Analysis code often jumps straight into tokenization without auditing placeholders and missingness patterns first.
...
- Category labels appear in the checkpoint/report without a cited mapping source.
- The same numeric ID is described differently in different checkpoint/report sections.
...
- Document the mapping source in the report appendix or methodology notes.

**Phase to address:** Phase 2 - Cleaning and Standardization

### Pitfall 10: Promising a video-length analysis when the CSV does not contain duration

**What goes wrong:** The assignment brief mentions video length, but the observed CSV header does not include a duration column. If the checkpoint/report promises length-based findings without external enrichment, that section will collapse late in the project.
...
- Analysis code starts inventing proxies such as title length or description length as if they were video length.
...
- The analysis uses file country as a direct proxy for channel origin.
...
### Pitfall 13: Checkpoint/report scope creep from trying to do all analyses at full depth

**What goes wrong:** The assignment asks for cleaning, timing analysis, category comparison, sentiment/NLP, and recommendations across all countries. Adding translation, topic modeling, clustering, dashboards, and extra modeling on top of that can produce a messy checkpoint/report flow with shallow conclusions.

**Why it happens:** Academic analysis artifacts often grow section by section without a locked question hierarchy.

**Consequences:** Late delivery, duplicated charts, weak narrative flow, and unfinished validation of the most important claims.

**Warning signs:**

- The report outline keeps expanding after the core questions are already covered.
...
- Reserve optional extras for an appendix only after the core checkpoint/report flow is coherent.

**Phase to address:** Phase 1 - Research Design and Scope Lock

### Pitfall 14: Weak storytelling between evidence and recommendations

**What goes wrong:** The analysis may show many charts but fail to answer the business-style question: what should a new channel actually do next?
...
- The analysis does not connect category, timing, and metadata findings into one coherent strategy.

**Prevention strategy:**

- Organize the checkpoint/report around the assignment questions, not around libraries or chart types.
...
| Final report/checkpoint bundle | Too many analyses, weak story | Keep one main storyline and move extras to an appendix |

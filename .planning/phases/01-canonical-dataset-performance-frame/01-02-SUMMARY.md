...
provides:
  ...
  - Markdown checkpoint contract test for proxy language, limitations, and rerun instructions
  - Phase 1 Markdown checkpoint documenting the row-level and video-country dataset contract
affects: [phase-02-feature-layer, phase-03-country-category-analysis, phase-04-text-analysis, phase-05-report]
tech-stack:
  added: []
  patterns: [first-trending anchor rows per country-video pair, explicit trending-corpus proxy naming, Markdown checkpoint contract tests]
key-files:
  created: [src/youtube_trends/performance_frame.py, tests/test_performance_frame.py, tests/test_phase1_checkpoint_contract.py, checkpoints/01_canonical_dataset.md]
...
key-decisions:
  ...
  - "Name trend_days_in_country_proxy explicitly as a trending-corpus proxy and carry the association-only framing into the Markdown checkpoint."
  - "Enforce Markdown checkpoint methodology and limitation language with a text-level contract test instead of manual-only review."
patterns-established:
  - "Snapshot contract: compute first and last trend dates plus first-snapshot metrics from the row-level parquet."
  - "Documentation guardrail: assert required headings and methodological caveats directly from the Markdown text."
...
# Phase 1 Plan 02: Performance Frame and Markdown Checkpoint Summary

**Video-country snapshot builder, explicit proxy framing, and machine-checkable Markdown checkpoint documentation**
...
- Added a Phase 1 Markdown checkpoint plus a text contract test that locks the proxy wording, association-only framing, missing-duration limitation, and rerun instructions.
...
4. **Task 2 checkpoint contract and documentation** - `6d3dd42` (feat)
...
- `tests/test_phase1_checkpoint_contract.py` - Reads the Markdown checkpoint and asserts the required headings, proxy language, limitations, and rerun commands.
- `checkpoints/01_canonical_dataset.md` - Documents the row-level table, snapshot table, performance proxy, limitations, and regeneration path.
...
- Treated the Markdown checkpoint as a contract artifact by testing its headings and caveat language directly, rather than leaving the methodology text to drift.
...
- The original interactive checkpoint was later replaced with a Markdown checkpoint so the artifact is cheaper to diff, summarize, and reuse in AI-driven workflows.
...
- Phase 1 is complete: the repo now has a canonical row-level parquet, a default video-country snapshot parquet, and a Markdown checkpoint that documents the dataset contract and proxy framing.
...
- The Markdown checkpoint and its contract test establish the language baseline future phases should preserve when discussing proxy validity, association limits, and missing video duration.

---
_Phase: 01-canonical-dataset-performance-frame_

...
- **D-08:** Name the primary target explicitly as a proxy within the trending corpus wherever it appears in the dataset and Markdown checkpoint.
...
- Whether the canonical build is documented through Markdown checkpoints and helper scripts.
...
- None yet — there are no existing Markdown checkpoints, scripts, source modules, or codebase maps to reuse in this repository.
...
- Phase 1 should read from `data/*.csv` and produce the canonical dataset contract that later Markdown checkpoints and final report work will consume.

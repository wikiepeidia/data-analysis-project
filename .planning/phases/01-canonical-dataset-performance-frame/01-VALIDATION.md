...
| **Quick run command** | `python -m pytest tests/test_canonical_dataset.py tests/test_performance_frame.py tests/test_phase1_checkpoint_contract.py -q` |
...
- **After every task commit:** Run the task-specific pytest file plus the phase quick command before a checkpoint/report handoff.
...
| 01-02-02 | 02 | 2 | DATA-03 | — | Markdown checkpoint documents the proxy, association framing, and missing-duration limitation. | integration | `python -m pytest tests/test_performance_frame.py tests/test_phase1_checkpoint_contract.py -q && python -m youtube_trends.performance_frame` | ❌ W0 | ⬜ pending |
...
- [ ] `tests/test_phase1_checkpoint_contract.py` — Markdown checkpoint contract checks for proxy documentation and limitations
